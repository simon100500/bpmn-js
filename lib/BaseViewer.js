...
/**
 * Update the global diagram font size and re-render all elements.
 *
 * @param {number|string} fontSize
 */
BaseViewer.prototype.setFontSize = function(fontSize) {
  this.get('textRenderer').setFontSize(fontSize);
  this._rerenderElements();
};

/**
 * Get the current global diagram font size.
 *
 * @return {number}
 */
BaseViewer.prototype.getFontSize = function() {
  return this.get('textRenderer').getDefaultStyle().fontSize;
};
...
BaseViewer.prototype._rerenderElements = function() {
  var elementRegistry = this.get('elementRegistry', false);
  ...
  this.get('eventBus').fire('elements.changed', { elements: elements });
};
