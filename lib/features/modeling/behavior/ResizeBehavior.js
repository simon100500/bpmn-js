 ...
 * @type {Dimensions}
 */
export var ACTIVITY_MIN_DIMENSIONS = { width: 100, height: 80 };
...
export default function ResizeBehavior(eventBus) {
  eventBus.on('resize.start', HIGH_PRIORITY, function(event) {
    ...
    else if (is(shape, 'bpmn:Group')) {
      context.minDimensions = GROUP_MIN_DIMENSIONS;
    }
    else if (is(shape, 'bpmn:Activity')) {
